...
function NovoAusenteTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ payload?: NovoAusenteChartPoint }>; label?: string | number }) {
  ...
  return (
    <div className="rounded-sm border border-slate-200 bg-white p-3 text-sm shadow-sm">
      ...
      <p className="tabular text-red-700">Ausentes : {formatBRL(point.ausentes)}</p>
      <p className="mt-2 tabular text-green-700">Novas : {formatBRL(point.novas)}</p>
      <p className={`mt-2 tabular ${point.total > 0 ? "text-red-700" : "text-green-700"}`}>Total : {formatBRL(point.total)}</p>
    </div>
  );
}
...
export function SyntheticPage({ profile }: { profile: Profile }) {
  ...
  const novoAusenteByMonth = useMemo(() => {
    ...
    for (let i = 1; i < availableMonths.length; i += 1) {
      ...
      const novas = novasItems.reduce((s, x) => s + Math.abs(x.total), 0);
      const ausentes = ausentesItems.reduce((s, x) => s + Math.abs(x.total), 0);

      chartData.push({
        mes: mesAtual,
        novas,
        ausentes,
        total: novas - ausentes,
        novasItems,
        ausentesItems,
      });
    }

...
export function AnalyticalPage({ profile }: { profile: Profile }) {
  ...
  const [mes, setMes] = useState(params.get("mes") ?? "todos");
  ...
  const [linkedRows, setLinkedRows] = useState<Array<any>>([]);
  const [page, setPage] = useState(1);
  const pageSize = 20;
  const linkedMode = params.get("linked") === "1";

  useEffect(() => {
    setMes(params.get("mes") ?? "todos");
  ...
  useEffect(() => {
    let active = true;
    if (!linkedMode || !conta || !mes || mes === "todos") {
      setLinkedRows([]);
      return;
    }
    ...
    void fetchAnalyticalLinkedRecords({
      ...
      if (!active) return;
      setLinkedRows(rows);
      const uniqueIds = new Set(rows.map((r) => String(r.recordId ?? "")).filter(Boolean));
      console.info("[Analitico linked]", {
        params: { conta, mes, batch: batch || null, userId: linkedUserId || null, setor: linkedSetor || null },
  ...
  }, [linkedMode, conta, mes, batch, linkedUserId, linkedSetor, profile.id, profile.role, data.adminSetor, data.adminUser]);

  const localFiltered = useMemo(() => {
    const normalizedConta = conta.trim().toLowerCase();
    const normalizedDescricao = descricao.trim().toLowerCase();
    return data.analyticalRows.filter((r) => (mes === "todos" || r.mes === mes)
      && (!normalizedConta || r.conta.toLowerCase().includes(normalizedConta))
      && (!normalizedDescricao || r.descricao.toLowerCase().includes(normalizedDescricao))
      && (forma === "todos" || r.forma === forma));
  }, [data.analyticalRows, mes, conta, descricao, forma]);

  const sourceRows = linkedMode ? linkedRows : localFiltered;
  ...
  useEffect(() => {
    ...
    const uniqueIds = new Set(deduped.map((r) => String(r.recordId ?? "")).filter(Boolean));
    console.info("[Analitico linked render]", {
      renderedRows: deduped.length,
      uniqueIds: uniqueIds.size,
      params: { conta, mes, batch: batch || null, userId: linkedUserId || null, setor: linkedSetor || null },
    });
  }, [linkedMode, deduped, conta, mes, batch, linkedUserId, linkedSetor]);
  ...
  const linkedSynthetic = useMemo(() => data.syntheticRows.filter((s) => {
    ...
    return contaOk
      && (mes === "todos" || s.mes === mes)
      && (!batch || s.uploadBatchId === batch);
  }), [data.syntheticRows, conta, mes, batch]);
  ...
  useEffect(() => {
    setPage(1);
  }, [mes, conta, descricao, forma, batch, linkedUserId, linkedSetor, data.adminSetor, data.adminUser, linkedMode]);
  ...
  const activeLinkedFilter = params.get("conta");

  return <div className="mx-auto max-w-[1400px] p-6 space-y-4">
    <header className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-bold">Analítico</h1><p className="text-slate-700 dark:text-slate-300">Lançamentos detalhados por Conta, Descrição, Valor, Forma e Mês.</p>{activeLinkedFilter && <p className="mt-1 text-sm text-green-700">Exibindo lançamentos da Conta {activeLinkedFilter}{mes && mes !== "todos" ? ` — ${mes}` : ""}</p>}</div><div className="flex gap-2"><Button className="bg-green-700 text-white" onClick={() => exportViewToXlsx({ sintetico: linkedSynthetic, analitico: sortedAnalytical, fileName: `analitico_${new Date().toISOString().slice(0,10)}.xlsx` })}>Exportar</Button><Button className="border border-[var(--border)] text-[var(--text)] bg-[var(--card)]" onClick={() => { setParams({}); setConta(""); setDescricao(""); setMes("todos"); setBatch(""); setLinkedUserId(""); setLinkedSetor(""); }}>Limpar filtro vinculado</Button></div></header>
